...
class SawyerButtonPressV2Policy(Policy):
    ...
    def get_action(self, obs):
        ...
        action["delta_pos"] = move(o_d["hand_pos"], to_xyz=self.desired_pos(o_d), p=25.0)
    ...
    def desired_pos(o_d):
        ...
        if not np.all(
            np.isclose(np.array([hand_x, hand_z]), np.array([button_initial_x, button_initial_z]), atol=0.02)
        ):
