...
class SawyerXYZEnv(SawyerMocapBase, metaclass=abc.ABCMeta):
    _HAND_SPACE = Box(np.array([-0.525, 0.348, -0.0525]), np.array([+0.525, 1.025, 0.7]))
    max_path_length = 500
    ...
    def _get_curr_obs_combined_no_goal(self):
        ...
        finger_right, finger_left = (self._get_site_pos("rightEndEffector"), self._get_site_pos("leftEndEffector"))
    ...
    def _get_state_rand_vec(self):
        if self._freeze_rand_vec:
            ...
            rand_vec = self.np_random.uniform(
                self._random_reset_space.low, self._random_reset_space.high, size=self._random_reset_space.low.size
            )
