...
class SawyerBinPickingEnvV2(SawyerXYZEnv):
    ...
    def evaluate_state(self, obs, action):
        (reward, near_object, grasp_success, obj_to_target, grasp_reward, in_place_reward) = self.compute_reward(
            action, obs
        )
    ...
    def compute_reward(self, action, obs):
        ...
        threshold = 0.03
        radii = [np.linalg.norm(hand[:2] - self.obj_init_pos[:2]), np.linalg.norm(hand[:2] - self._target_pos[:2])]
        ...
        return (reward, near_object, grasp_success, target_to_obj, object_grasped, in_place)
