...
class SawyerLeverPullEnvV2(SawyerXYZEnv):
    ...
    def evaluate_state(self, obs, action):
        ...
        (reward, shoulder_to_lever, ready_to_lift, lever_error, lever_engagement) = self.compute_reward(action, obs)
    ...
    def compute_reward(self, action, obs):
        ...
        lever_engagement = reward_utils.tolerance(
            lever_error, bounds=(0, np.pi / 48.0), margin=(np.pi / 2.0) - (np.pi / 12.0), sigmoid="long_tail"
        )
        ...
        return (reward, np.linalg.norm(shoulder_to_lever), ready_to_lift, lever_error, lever_engagement)
