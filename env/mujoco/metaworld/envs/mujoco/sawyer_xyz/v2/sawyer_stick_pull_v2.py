...
class SawyerStickPullEnvV2(SawyerXYZEnv):
    ...
    def _get_quat_objects(self):
        return np.hstack(
            (Rotation.from_matrix(self.data.get_body_xmat("stick")).as_quat(), np.array([0.0, 0.0, 0.0, 0.0]))
        )
    ...
    def compute_reward(self, action, obs):
        ...
        stick_in_place_margin = np.linalg.norm((self.stick_init_pos - container_init_pos) * yz_scaling)
