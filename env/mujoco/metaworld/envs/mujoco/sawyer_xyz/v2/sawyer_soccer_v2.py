...
class SawyerSoccerEnvV2(SawyerXYZEnv):
    ...
    def evaluate_state(self, obs, action):
        obj = obs[4:7]
        (reward, tcp_to_obj, tcp_opened, target_to_obj, object_grasped, in_place) = self.compute_reward(action, obs)
    ...
    def compute_reward(self, action, obs):
        ...
        if target_to_obj < self.TARGET_RADIUS:
            reward = 10.0
        return (reward, tcp_to_obj, tcp_opened, np.linalg.norm(obj - self._target_pos), object_grasped, in_place)
