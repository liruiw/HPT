...
MT10_V2_ARGS_KWARGS = {
    key: dict(args=[], kwargs={"task_id": list(ALL_V2_ENVIRONMENTS.keys()).index(key)}) for key, _ in MT10_V2.items()
}
...
MT50_V2_ARGS_KWARGS = {
    key: dict(args=[], kwargs={"task_id": list(ALL_V2_ENVIRONMENTS.keys()).index(key)}) for key, _ in MT50_V2.items()
}
...
def create_observable_goal_envs():
    ...
    for env_name, env_cls in ALL_V2_ENVIRONMENTS.items():
        ...
        d["__init__"] = initialize
        og_env_name = re.sub("(^|[-])\s*([a-zA-Z])", lambda p: p.group(0).upper(), env_name)
