...
def test_all_ml1(env_name):
    ...
    for rand_vecs in test_env_rand_vecs.values():
        for rand_vec in rand_vecs:
            train_test_rand_vecs.add(tuple(rand_vec))
    assert (
        len(train_test_rand_vecs) == (len(ml1.test_classes.keys()) + len(ml1.train_classes.keys())) * metaworld._N_GOALS
    )
...
def test_all_ml10():
    ...
    assert (
        ...
        == (len(ml10.test_classes.keys()) + len(ml10.train_classes.keys())) * metaworld._N_GOALS
    )
    del test_env_instances
...
def test_all_ml45():
    ...
    assert (
        len(train_test_rand_vecs)
        == (len(ml45.test_classes.keys()) + len(ml45.train_classes.keys())) * metaworld._N_GOALS
    )
    del test_env_instances
...
def test_all_mt10():
    mt10 = MT10()
    train_env_instances = {env_name: env_cls() for (env_name, env_cls) in mt10.train_classes.items()}
...
def test_all_mt50():
    mt50 = MT50()
    train_env_instances = {env_name: env_cls() for (env_name, env_cls) in mt50.train_classes.items()}
...
def check_tasks_unique(tasks, env_names):
    ...
    for env_name in env_names:
        env_to_rand_vecs[env_name] = np.array(
            [pickle.loads(task.data)["rand_vec"] for task in tasks if (task.env_name == env_name)]
        )
