...
class DDPMScheduler:
    ...
    def set_timesteps(
        ...
        if timesteps is not None:
            ...
            if timesteps[0] >= self.num_train_timesteps:
                raise ValueError(
                    f"`timesteps` must start before `self.train_timesteps`:" f" {self.num_train_timesteps}."
                )
    ...
    def step_mcmc(
        ...
        # 6. Add noise\
        noise = torch.randn(shape=pred_original_sample).to(pred_original_sample)
        noisy_new_sample = self.add_noise(pred_original_sample, noise, t)  # torch

        return DDPMSchedulerOutput(prev_sample=noisy_new_sample, pred_original_sample=pred_original_sample)
